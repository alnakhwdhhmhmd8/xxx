...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "wwvvwl":
      try:
         ...
         await client.set_administrator_title(chat_id, user_id, "جاك")
      except:
        pass
      return await message.reply_text(f"**انضم المطو جاك للشات  🚦 .\nمرحبا بك : @wwvvwl  🚦 .**")
    dev = await get_dev(bot_username)
...
async def wwvvwl(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
"مهندس برمجة اتجوز وخلف بنتين توأم.. سمى واحدة لوجين والتانية Log out 👻😹" , 
"طجاكي ضاعت وانا بعد الكشكول ابو 60 ورقه عشان اشوف الشركة نصابة ولا لا 👻😹", 
...
"مره واحد اسمو جاك  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
...
"لا جاكي على الدوق فيما يحب أو بكره",
...
async def alive(client: Client, message):
    ...
    alive = f"""╭──── • ◈ • ────╮
么 [𝗦𝗢𝗨𝗥𝗖𝗘 𝗔𝗟𝗤𝗔𝗜𝗗](t.me/xvwwvl) 🔰 .
么 [𝐌𝐎𝐑𝐀𝐄𝐁](t.me/wwvvwl) 👑 .
    ...
    await message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                ...
                [
                    InlineKeyboardButton("√ َِ𝐌𝐎𝐑𝐀𝐄𝐁™|| » مـرعـب ", user_id=f"wwvvwl")
                ],
                [
                    InlineKeyboardButton("اضف البوت الي مجموعتك ❤️", url="https://t.me/{bot.username}?startgroup=true")
                ],
...
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="wwvvwl")
...
@Client.on_message(filters.command(["جاكي","مطور السورس","ماندو"], ""))
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["wwvvwl"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["wwvvwl"]:
   ...
   if message.from_user.username == "wwvvwl":
     await message.reply_text(f"**تم تنفيذ الامر بواسطة المطور جاك  🚦 .**")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**➧ ᴊᴀᴋ |⌯ @xvwwvl  🚦 .**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**➧ ᴊᴀᴋ |⌯ @xvwwvl  🚦 .**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**➧ ᴊᴀᴋ |⌯ @xvwwvl  🚦 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["wwvvwl"]:
         await message.reply_text("**رتبتك هي : المطور جاك  🚦 .**")
         return
     if userr.username in ["wwvvwl"]:
         await message.reply_text("**رتبتك هي : المطور جاكي 🚦 .**")
         return
     if userr.id == dev:
