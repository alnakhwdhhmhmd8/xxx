...
from Gold.Data import get_data
from googletrans import Translator
from Gold.Data import (get_call, get_app, get_userbot, get_group, get_channel, must_join)
    ...
    try:
        ...
        for result in (await results.next())["result"]:
            try:
                ...
                title = title.title()
                title = (await translator.translate(title, dest="en")).text  # تصحيح await
            except:
        ...
        song_cover = Image.open(f"thumb{videoid}.png")
        song_cover = changeImageSize(520, 520, song_cover)

        
        mask = Image.new("L", (520, 520), 0)
        draw_mask = ImageDraw.Draw(mask)
        draw_mask.ellipse((0, 0, 520, 520), fill=255)
        song_cover.putalpha(mask)

        
        border_color = random.choice(["red", "blue", "green", "yellow", "purple", "orange"])
        border = Image.new("RGBA", (550, 550), (0, 0, 0, 0))
        draw_border = ImageDraw.Draw(border)
        draw_border.ellipse((0, 0, 550, 550), outline=border_color, width=15)

        
        border.paste(song_cover, (15, 15), mask=song_cover)
        background.paste(border, (50, 100), mask=border)
...
async def download(bot_username, link, video: Union[bool, str] = None):
        ...
        def audio_dl():
            ydl_optssx = {"format": "bestaudio/best", "outtmpl": f"downloads/{bot_username}%(id)s.%(ext)s", "geo_bypass": True, "nocheckcertificate": True, "quiet": True, "no_warnings": True, "cookiefile": cookies_file}
            ...
            return xyz
        if video:
            proc = await asyncio.create_subprocess_exec("yt-dlp", "-g", "-f", "best[height<=?720][width<=?1280]", f"{link}", "--cookies", cookies_file, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
