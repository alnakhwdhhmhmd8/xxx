...
from Gold.info import Call, activecall, helper, active
from Gold.Data import db, dev, devname, set_must, get_data
  ...
  for i in bots:
      ...
      try:
       if not i["bot_username"] in Done:
        ...
        logger = i["logger"]
        bot = Client("Gold", api_id=API_ID, api_hash=API_HASH, bot_token=TOKEN, in_memory=True, plugins=dict(root="Gold"))
        user = Client("Gold", api_id=API_ID, api_hash=API_HASH, session_string=SESSION, in_memory=True)
        await bot.start()
        await user.start()
...
@app.on_message(filters.command(["تفعيل المجاني", "تعطيل المجاني"], ""))
async def bye(client, message):
    user = message.from_user.username
    if user in OWNER:
        global OFF
        text = message.text
        if text == "تفعيل المجاني":
            OFF = None
            await message.reply_text("**♪ تم تفعيل الوضع المجاني  ⚡ .**",quote=True)
            return
        if text == "تعطيل المجاني":
            OFF = True
...
["اذاعه للمطورين","المكالمات النشطه"]], resize_keyboard=True)
    ...
    kep = ReplyKeyboardMarkup([
["حذف بوت", "صنع بوت"]], resize_keyboard=True)
...
async def alive(client: Client, message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("𝐺𝑟𝑜𝑢𝑝", url=f"{gr}"),
                InlineKeyboardButton("𝐶ℎ𝑎𝑛𝑛𝑒𝑙", url=f"{ch}"),
            ],
            [
                 InlineKeyboardButton(f"{OWNER_NAME}", url=f"https://t.me/{OWNER[0]}")
            ],
            [ 
                 InlineKeyboardButton("اضف البوت الي مجموعتك ⚡", url="https://t.me/{bot.username}?startgroup=True")
            ]
        ]
    )

    alive = f"""⍟ 𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼"""
...
async def cloner(app: app, message):
    ...
    if OFF:
      if not message.chat.username in OWNER:
         return await message.reply_text(f"**● الوضع المجاني مغلق الان  ⚡ .\n● راسل المطور لتنصيب مدفوع  😎 .\n♪ Dev : @{OWNER[0]}  ⚡ .**")
